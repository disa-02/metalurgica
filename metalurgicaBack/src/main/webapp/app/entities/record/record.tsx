  ...
  return (
    <div>
      <h2 id="record-heading" data-cy="RecordHeading">
        <Translate contentKey="metalurgicaBckApp.record.home.title">Records</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="metalurgicaBckApp.record.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/record/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="metalurgicaBckApp.record.home.createLabel">Create new Record</Translate>
          </Link>
        </div>
      </h2>
        ...
        {recordList && recordList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="metalurgicaBckApp.record.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {recordList.map((record, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{record.dateRange ? <TextFormat type="date" value={record.dateRange} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{record.amount}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="metalurgicaBckApp.record.home.notFound">No Records found</Translate>
            </div>
          )
        )}
      </div>
